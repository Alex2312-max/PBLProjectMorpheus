...
def processing_view(request, id):
    ...
    user_model.objects.filter(id=id).update(bin_data=pick)
    ...
    generated_schedule = gap.generate(day_schedule, 70/1.75**2)
    slept_hours = sum(generated_schedule)
    print(generated_schedule)
    print(f'IDS - {gap.ids}')
    print(f'SQ - {gap.sleep_quality}')

    users = get_user_model()
    user = users.objects.all()[id-1]
    ...
    return HttpResponseRedirect(f'/userPage/{id-1}')